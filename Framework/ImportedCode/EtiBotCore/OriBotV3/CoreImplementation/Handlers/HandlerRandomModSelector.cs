...
namespace OldOriBot.CoreImplementation.Handlers
{
    public class HandlerRandomModSelector : PassiveHandler
    {
        ...
        private Task OnPresenceUpdated(Presence _, Presence np)
        {
            ...
            IEnumerable<Member> mods = oriContext.Server.FindMembersWithRole(oriContext.Server.GetRole(1139468796449534043));
            mods = mods.Where(mod =>
            {
                return (mod.Presence.Status == StatusType.Online || mod.Presence.Status == StatusType.Idle) && !mod.Roles.Contains(1194519633647124520);
            });
